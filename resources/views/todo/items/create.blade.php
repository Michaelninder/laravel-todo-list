            ...
            <form action="{{ route('todo.items.store', $list->id) }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <select id="state" name="state" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-300 focus:ring focus:ring-primary-200 focus:ring-opacity-50 dark:bg-zinc-800 dark:border-zinc-600 dark:text-white">
                        <option value="open" {{ old('state', 'open') === 'open' ? 'selected' : '' }}>{{ __('Open') }}</option>
                        <option value="in_progress" {{ old('state') === 'in_progress' ? 'selected' : '' }}>{{ __('In Progress') }}</option>
                        <option value="done" {{ old('state') === 'done' ? 'selected' : '' }}>{{ __('Done') }}</option>
                        <option value="cancelled" {{ old('state') === 'cancelled' ? 'selected' : '' }}>{{ __('Cancelled') }}</option>
                    </select>
                    @error('state')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
